    ...
    <div class="py-12">
        <div class="mx-auto max-w-6xl sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex items-center justify-between">
                ...
                <a href="{{ route('technical-adviser.team-leaders.create') }}" class="inline-flex items-center justify-center rounded-lg bg-orange-600 px-6 py-3 text-sm font-semibold text-white hover:bg-orange-700">
            ...
            <div class="bg-white overflow-hidden shadow-sm rounded-2xl ring-1 ring-slate-200">
                            ...
                            <tbody class="divide-y divide-slate-200">
                                @foreach ($teamLeaders as $leader)
                                    <tr class="hover:bg-slate-50 transition">
                                        <td class="px-6 py-4 text-sm text-slate-900">{{ $leader->name }}</td>
                                        <td class="px-6 py-4 text-sm text-slate-600">{{ $leader->email }}</td>
                                        <td class="px-6 py-4 text-sm">
                                            @if ($leader->deleted_at)
                                        ...
                                        <td class="px-6 py-4 text-sm text-right">
                                            <div class="flex items-center justify-end gap-2">
                                                ...
                                                <form id="delete-form-{{ $leader->id }}" action="{{ route('technical-adviser.team-leaders.destroy', $leader) }}" method="POST" class="hidden">
                                                    @csrf
                                                    @method('DELETE')
                                                </form>
                                            </div>
                ...
                @else
                    <div class="p-6 text-center">
                        ...
                        <a href="{{ route('technical-adviser.team-leaders.create') }}" class="mt-4 inline-flex items-center justify-center rounded-lg bg-orange-600 px-6 py-3 text-sm font-semibold text-white hover:bg-orange-700">
